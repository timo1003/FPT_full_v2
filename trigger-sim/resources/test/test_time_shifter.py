...
def TestSetup(frame, add_empty_trigger = False, add_mchits = True ) :
    ...
    pulsemap[icetray.OMKey(21,30)] = pulsemap
    frame["RecoPulseMap"] = pulsemap
...
def TestShift(frame, expect_mchits = True, expect_trigger = True):
    ...
    for t in frame["TriggerVector"] :
        ...
        assert t.time == 0, "I3TriggerVector"

    # I3TriggerHierarchy
    if expect_trigger :
        for t in frame["Trigger"] :
            print("TriggerH t.time = ",t.time)
            assert t.time == 0, "I3Trigger"

    # I3FlasherInfoVector
    for fi in frame["FlasherInfos"] :
        print("FlasherInfo fi.flash_time = ",fi.flash_time)
        assert fi.flash_time == 0, "I3FlasherInfo"

    # I3DOMLaunch
    for om,ls in frame["DOMLaunchMap"] :
        for l in ls :
            print("I3DOMLaunch l.time = ",l.time)
            assert l.time == 0, "I3DOMLaunch"

    # I3RecoPulse
    for om,ps in frame["RecoPulseMap"] :
        for p in ps :
            print("I3RecoPulse p.time = ",p.time)
            assert p.time == 0, "I3RecoPulse"
    ...
    for mmctrack in frame["MMCTracks"] :
        ...
        assert mmctrack.ti == 0., "I3MMCTrack.ti = %f" % mmctrack.ti
        assert mmctrack.tc == 0., "I3MMCTrack.tc = %f" % mmctrack.tc
        assert mmctrack.tf == 0., "I3MMCTrack.tf = %f" % mmctrack.tf
        assert mmctrack.GetI3Particle().time == 0., \
                      "I3MMCTrack.I3Particle.time = %f"\
                        % mmctrack.GetI3Particle().time

    # I3Double
    assert frame["SomeTime"].value == 0., \
                  "SomeTime value = %f" % frame["SomeTime"].value
    assert frame["NotTime"].value == TIME, \
