...
class TestSourcePulses(icetray.I3Module):

    def __init__(self, context):
        icetray.I3Module.__init__(self, context)
        self.AddParameter('NPMTs', '', 100)
        self.AddParameter('TimeWindow', '', 1000.)
        self.AddParameter('GenerateInTimeWindow', '', True)
        self.AddParameter('PMTType', '', dataclasses.I3OMGeo.mDOM)
        self.AddParameter('PulseMapName', '', 'I3RecoPulseSeriesMapExtensions')
        self.AddOutBox("OutBox")

    def Configure(self):
        self.NPMTs = self.GetParameter('NPMTs')
        self.timeWindow = self.GetParameter('TimeWindow')
        self.generateInTimeWindow = self.GetParameter('GenerateInTimeWindow')
        self.pmtType = self.GetParameter('PMTType')
    ...
    def DAQ(self, frame):
        ...
        for i in range(self.NPMTs):
            ...
            time = random.uniform(0,self.timeWindow)
            if not self.generateInTimeWindow :
                time += pulse_counter * self.timeWindow
            ...
            if omkey not in pulsemap :
                pulsemap[omkey] = dataclasses.I3RecoPulseSeries()

            pulsemap[omkey].append(pulse)

            pulse_counter += 1

        frame[self.pulseMapName] = pulsemap
        self.PushFrame(frame)
...
class TestModule(icetray.I3Module):
    ...
    def __init__(self, context):
        ...
        self.AddParameter('TriggerSourceID', '', dataclasses.IN_ICE)
        self.AddParameter('TriggerTypeID', '', dataclasses.SIMPLE_MULTIPLICITY)
