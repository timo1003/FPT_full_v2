...
def n_triggers(th, sourceID = None, typeID = None, configID = None):
    if sourceID is None and \
            typeID is None and \
            configID is None : return 0
    return_value = 0
    for t in th:
        if ( sourceID is None or t.key.source == sourceID ) and \
        ( typeID is None or t.key.type == typeID ) and \
        ( configID is None or t.key.config_id == configID ) :
            return_value += 1
    return return_value      
...
def trigger_lengths(th, sourceID = None, typeID = None, configID = None):
    ...
    if sourceID is None and \
            typeID is None and \
            configID is None : return return_list
    for t in th:
        if ( sourceID is None or t.key.source == sourceID ) and \
        ( typeID is None or t.key.type == typeID ) and \
        ( configID is None or t.key.config_id == configID ) :
