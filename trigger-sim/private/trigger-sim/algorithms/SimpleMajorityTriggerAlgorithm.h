...
/**
   ...
   A trigger is satisfied if there are at least TriggerThreshold hits in a time
   ...
   1) Find a time window: A sliding time window is applied to each hit and the 
      ...
      is met, the window will continue to slide. Once the number of hits drops
   ...
   A time window with multiple independent clusters of hits will only produce a single
...
class SimpleMajorityTriggerAlgorithm : public TriggerService
{
 ...
 public:
  SimpleMajorityTriggerAlgorithm(double triggerWindow, 
