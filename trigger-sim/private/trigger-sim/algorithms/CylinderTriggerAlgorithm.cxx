  ...
  Height_(Height)
{
  ...
  log_debug("  Height = %g", Height_);

  hitQueue_.clear();
  Geometry_ = Geometry;
}

void CylinderTriggerAlgorithm::Trigger()
{

  log_debug("CylinderTrigger has %zd hits to process", hits_->size());
  ...
  for (nextHit = hits_->begin(); nextHit != hits_->end(); nextHit++) {
    ...
    double stopTime __attribute__((unused)) = startTime + triggerWindow_;
