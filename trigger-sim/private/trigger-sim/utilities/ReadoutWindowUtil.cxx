  ...
  bool sameSubdetector = ( ( (triggerSource == TriggerKey::IN_ICE) && 
                           ...
                           ( (triggerSource == TriggerKey::ICE_TOP) && 
                             ( (subdetector == I3TriggerStatus::ICETOP) || 
                               (subdetector == I3TriggerStatus::ALL) ) ) ||
                           ( (triggerSource == TriggerKey::IN_ICE_PULSES) && 
                             ( (subdetector == I3TriggerStatus::INICE) || 
                               (subdetector == I3TriggerStatus::ALL) ) ) );

  double readoutStart = triggerStart - readoutConfig.readoutTimeMinus;
  double readoutStop = triggerStop + readoutConfig.readoutTimePlus;
  if (!sameSubdetector) {
    // This readout is for another subdetector, so we apply an offset.
    readoutStart += readoutConfig.readoutTimeOffset;
    readoutStop += readoutConfig.readoutTimeOffset;
  }
