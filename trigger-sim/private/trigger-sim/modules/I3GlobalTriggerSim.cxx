...
I3GlobalTriggerSim::I3GlobalTriggerSim(const I3Context& ctx) 
  : I3Module(ctx),
    ...
    time_increment_(0.),
    run_id_(std::numeric_limits<unsigned>::max()),
    dt_(0.)
{
   ...
   recoPulseSeriesMapNames_.push_back("I3RecoPulseSeriesMapExtensions");
   AddParameter("I3RecoPulseSeriesMapNames",
		"This holds the I3RecoPulses used to determine event lengths for untriggered events in"
                " the Upgrade and Gen2 volumes.",
		recoPulseSeriesMapNames_);
...
void I3GlobalTriggerSim::Configure()
{
   ...
   GetParameter("I3RecoPulseSeriesMapNames", recoPulseSeriesMapNames_);
   ...
   if(run_id_ == std::numeric_limits<unsigned>::max()){
...
void I3GlobalTriggerSim::DAQ(I3FramePtr frame)
{
   ...
   if(gTriggers && gTriggers->size()){
     ...
     BOOST_FOREACH(std::string& s,recoPulseSeriesMapNames_){
       I3RecoPulseSeriesMapConstPtr old_pulsemap = frame->Get<I3RecoPulseSeriesMapConstPtr>(s);
       if(old_pulsemap){
         BOOST_FOREACH(I3RecoPulseSeriesMap::const_reference pair, *old_pulsemap){
           BOOST_FOREACH(const I3RecoPulse& p, pair.second){
             start_times.push_back(p.GetTime()-p.GetWidth());
             stop_times.push_back(p.GetTime()+p.GetWidth());
           }
         }
       }else{
         log_debug("There was no I3RecoPulse map %s in the frame.", s.c_str());
       }
     }
   }
   ...
   end_time = start_time + latest - earliest;
