...
TriggerCheck_13::TriggerCheck_13(const I3Context& context)
  : I3Module(context),
    ...
    dmice_smt_bool_("DMIceSMTTriggered")
{
  AddOutBox("OutBox");
  ...
  AddParameter("IceActSMTTriggerFlag",
  	       ...
  	       iceact_smt_bool_);
  AddParameter("DMIceSMTTriggerFlag",
  	       "Name of bool to use when an IN_ICE::SMT DM Ice Trigger is present",
  	       dmice_smt_bool_);
  ...
  AddParameter("IceActSMTConfigID",
	       ...
	       iceact_smt_config_);
  dmice_smt_config_ = 99999;
  AddParameter("DMIceSMTConfigID",
	       "The config ID used for DM Ice simple multiplicity triggers",
	       dmice_smt_config_);
 ...
 void TriggerCheck_13::Configure()
{
  ...
  GetParameter("IceActSMTTriggerFlag",
	       iceact_smt_bool_);
  GetParameter("DMIceSMTTriggerFlag",
	       dmice_smt_bool_);
...
**/
  ...
  unsigned int DMIceSMT_trigger = Count(*triggers,
					 TriggerKey(TriggerKey::IN_ICE,
						    TriggerKey::SIMPLE_MULTIPLICITY,
						    dmice_smt_config_));
  ...
  // Ditto for DMIce SMT
  if(DMIceSMT_trigger > 0)
    inice_smt = inice_smt - DMIceSMT_trigger;
  ...
  I3BoolPtr DMIceSMT_boolPtr(new I3Bool(false));
  ...
  if (inice_smt)
   ...
   if (DMIceSMT_trigger)
     {
       DMIceSMT_boolPtr->value = true;
       log_trace("DMIceSMT True");
     }
   frame->Put(dmice_smt_bool_,DMIceSMT_boolPtr);
   if (faint_part){
