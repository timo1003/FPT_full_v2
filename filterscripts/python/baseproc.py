...
def BaseProcessing(tray, name, pulses='MuonPulseSeriesReco',
                   ...
                   seededRTConfig=None,
                   do_icetop_tankmerger_prep=False):
     """
     ...
     if decode:
          tray.AddSegment(payload_parsing.I3DOMLaunchExtractor,
                          ...
                          ## location of scintillators, IceACT and DMIce
                          SpecialDataOMs = [OMKey(0,1),    # 2016, IceACT
                                            OMKey(0,2),    # 2022, DMIce
                                            OMKey(0,3),    # 2022, DMIce
                                            OMKey(0,4),    # 2022, DMIce
                                            OMKey(0,5),    # 2022, DMIce
     ...
     if do_icetop_tankmerger_prep:
          # KR adds: Starting with the (complete) IceTop pulses extracted from SuperDST, we've gotta
          # convert them to VEM, and split them up into HLC and SLC, before running TankPulseMerger.
          # This is a uniquely "pass3" thing, and has to happen after the IceTopDSTPulses have been created,
          # but before the OnlineFilter (which will use the TankPulses)
          from icecube.filterscripts.offlineL2.level2_IceTop_CalibrateAndExtractPulses import IceTopWaveformSplitter
          IceTopPulses = 'IceTopDSTPulses'
          tray.AddModule('I3VEMConverter', name + '_northern_VEMConverter',
                         PEPulses=IceTopPulses,
                         VEMPulses='IceTopVEMPulses',
                         If=lambda frame: IceTopPulses in frame)
          
          tray.AddModule(IceTopWaveformSplitter, name + '_northern_IceTopSplitterIntoHLCSLC',
                         Input='IceTopVEMPulses',
                         OutputHLC='IceTopHLCVEMPulses', # not filter_globals.IceTopPulses_HLC, which are the old "PE" version
                         OutputSLC='IceTopSLCVEMPulses', # not filter_globals.IceTopPulses_SLC, similarly
                         If=lambda frame: IceTopPulses in frame)
     
     #tray.AddModule("Dump", "dumpme")
     tray.AddModule('I3TankPulseMerger', 'IceTopTankPulseMerger_HLC',
                     ...
                     If               = lambda frame: 'IceTopHLCVEMPulses' in frame
                   )
     ...
     tray.AddModule('I3TankPulseMerger', 'IceTopTankPulseMerger_SLC',
                     ...
                     MaxHGLGTimeDiff  = 40.0 * I3Units.ns,               # Default
                     If               = lambda frame: 'IceTopSLCVEMPulses' in frame
                   )
     ...
     tray.AddModule("TriggerCheck_13",name + "_Trigchecker",
                    ...
                    DMIceSMTConfigID=filter_globals.dmicesmtconfigid
                    )
