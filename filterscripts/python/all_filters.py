...
def OnlineFilter(tray, name, simulation=False, decode = False, If=lambda f: True,
    ...
    from icecube.filterscripts.icetop_SW1tank import IceTopFilterSelectedStations
    ...
    from icecube.filterscripts.dmicesmtfilter import DMIceTrigFilter
...
##     PoleMuonLineit, PoleMuonLlh, Cuts module and Mue on PoleMuonLlh
    ...
    if sdstarchive:
        tray.AddSegment(BaseProcessing, "BaseProc",
                        ...
                        do_icetop_tankmerger_prep=True  # because starting in IC86.2023, this hasn't been done yet with SDST from Pole
                        )
    else:
        tray.AddSegment(BaseProcessing, "BaseProc",
                        pulses=filter_globals.CleanedMuonPulses,
...
## KR: this is already done at Pole (as of 2022), so not needed again -- set the "vemcal_enabled"
## option to "false" when you call this segment in the North.
    if vemcal_enabled:
...
## already e.g. SRT cleaned, such as CleanedMuonPulses.
    ...
    tray.AddSegment(DMIceTrigFilter, "DMIceFilters",
                    If = which_split(split_name=filter_globals.NullSplitter)
                    )
    ...
    tray.AddSegment(IceTopFilterSelectedStations, "1TankSWStationFilter",
                    If = which_split(split_name=filter_globals.IceTopSplitter)
                    )
