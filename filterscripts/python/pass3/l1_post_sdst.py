...
def sdst_to_l1(tray, name, ic79_geometry=False):
    ...
    # (KR comments this back in, so that the waveforms successfully get "found" when it's time to save them.
    #  Also, make sure the scintillators and other special DOM's don't sneak in
    tray.AddSegment(payload_parsing.I3DOMLaunchExtractor,
                    ...
                    HeaderID = '',
                    InIceID = 'UnusedInIce',
                    #MinBiasID='MinBias',  # are these necessary?
                    #FlasherDataID='Flasher',
                    #CPUDataID="BeaconHits",
                    SpecialDataID="SpecialHits",
                    # location SpecialOMs: DMIce, scintillators and IceACT
                    SpecialDataOMs=[icetray.OMKey(0, 1),     # 2016, IceACT
                                    icetray.OMKey(0, 2),     # 2022, DMIce
                                    icetray.OMKey(0, 3),     # 2022, DMIce
                                    icetray.OMKey(0, 4),     # 2022, DMIce
                                    icetray.OMKey(0, 5),     # 2022, DMIce
                                    icetray.OMKey(12, 65),   # 2016, Scintillator
                                    icetray.OMKey(12, 66),   # 2016, Scintillator
                                    icetray.OMKey(62, 65),   # 2016, Scintillator
                                    icetray.OMKey(62, 66)],  # 2016, Scintillator
                    If = lambda frame: "IceTopRawData" not in frame
                    )
    ...
    tray.AddSegment(OnlineFilter, "OnlineFilter",
                    ...
                    alert_followup=False,            # not needed for offline
                    alert_followup_omit_GCD_diff=True,
                    forceOnlineL2BadDOMList="BadDomsList", # formerly "BadDomsList_old", 
                    ic79_geometry = ic79_geometry,
                    #gfu_enabled=False # to run without ROOT
                    vemcal_enabled=False  # As of 2022, creation of I3VEMCalData and I3ITSLCCalData is done at pole already
    )
