...
def get_i3live_info(run_id, i3live_host='https://live.icecube.wisc.edu'):
    ...
    if j['grl_start']:
        start = datetime.strptime(j['grl_start'], '%Y-%m-%d %H:%M:%S').replace(tzinfo=timezone.utc)
        start_frac = j['grl_start_frac']
    else:
        start = datetime.strptime(j['start'], '%Y-%m-%d %H:%M:%S').replace(tzinfo=timezone.utc)
        start_frac = j['start_frac']
    if j['grl_stop']:
        stop = datetime.strptime(j['grl_stop'], '%Y-%m-%d %H:%M:%S').replace(tzinfo=timezone.utc)
        stop_frac = j['grl_stop_frac']
    else:
        stop = datetime.strptime(j['stop'], '%Y-%m-%d %H:%M:%S').replace(tzinfo=timezone.utc)
