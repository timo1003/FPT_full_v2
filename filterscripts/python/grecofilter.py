...
def nchannel_cut(frame, name,
    ...
    if name + pulsename not in frame.keys(): return False
    if name + spe_pulses not in frame.keys(): return False
    if name + fillratio_name not in frame.keys(): return False
...
def GRECOOnlineFilter(tray, name,
    ...
    def cleaning_if(frame):
        return TestForFilter(frame) & If(frame)
